...
class GUMP
{
    ...
    */
    ...
    /**
     * Check if the specified key is unique.
     *
     * Usage: '<index>' => 'required'
     *
     * @param string $field
    ...
    protected function validate_unique($field, $input, $param = null)
    {
        ...
        if(!$this->db->has($param,[$field => $input[$field]])){
            return true;
        }
     ...
     * @return mixed
     */
    protected function validate_not_null($field, $input, $param = null)
    {
        if(!isset($input[$field])){
            return;
        }
    ...
    {
        ##########################################################################
        ################Modified by jnbruno to work properly######################
        ##########################################################################
    ...
    public function validateParameters(array $param,array $validParam, &$error, $ignore_required = false)
    {
        $error = false;
            ...
            if(is_array($value)){
                ...
                if(is_array($value)){
                    if(empty($value[0]))
                    {
                        ...
                        $rule[$key] = $ignore_required ? str_replace("required", "not_null", $value[0]) : $value[0];
                    }
                ...
                if (!empty($value)){
                    // $rule[$key] = $value;
                    $rule[$key] = $ignore_required ? str_replace("required", "not_null", $value) : $value;
                } else {
                    $rule[$key] = 'validParam';
                }
            }
